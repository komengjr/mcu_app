            ...
            <div class="card bg-200 shadow border border-primary">
                <div class="row gx-0 flex-between-center">
                    <div class="col-sm-auto d-flex align-items-center border-bottom">
                        <img class="ms-3 mx-3" src="{{ asset('img/akses1.png') }}" alt="" width="50" />
                        <div>
                    ...
                    <div class="col-xl-auto px-3 py-2">
                        ...
                        <h4 class="text-primary fw-bold mb-0">Master <span class="text-info fw-medium">Menu Access</span>
                        </h4>
                    </div>
                </div>
            </div>
    ...
    <div class="card mb-3">
        <div class="card-header bg-primary">
            <div class="row align-items-center">
                <div class="col">
                    <h3 class="m-0"><span class="badge bg-primary m-0 p-0">Master Menu Access</span></h3>
                </div>
                <div class="col-auto">
                ...
                <tbody>
                    ...
                    @foreach ($data as $datas)
                        <tr>
                            ...
                            <td>
                                @php
                                    $check = DB::table('z_menu_user')
                                        ->join('z_menu_sub', 'z_menu_sub.menu_sub_code', 'z_menu_user.menu_sub_code')
                                        ->where('z_menu_user.access_code', $datas->master_access_code)
                                        ->get();
                                @endphp
                                @foreach ($check as $checks)
                                    <li>{{ $checks->menu_sub_name }}</li>
                                @endforeach
                            </td>
                            <td>
                                <div class="btn-group" role="group">
                                    ...
                                    <div class="dropdown-menu" aria-labelledby="btnGroupVerticalDrop2">
                                        <button class="dropdown-item" data-bs-toggle="modal" data-bs-target="#modal-menu-lg"
                                            id="button-setting-access" data-code="{{ $datas->master_access_code }}"><i
                                                class="fas fa-universal-access"></i></span> Setiing Aksess
                                        </button>
...
@section('base.js')
    ...
    <div class="modal fade" id="modal-menu-lg" data-bs-keyboard="false" data-bs-backdrop="static" tabindex="-1"
        aria-labelledby="staticBackdropLabel" aria-hidden="false">
    ...
    <script>
        $(document).on("click", "#button-setting-access", function(e) {
            e.preventDefault();
            ...
            $.ajax({
                url: "{{ route('master_menu_access_setting') }}",
                type: "POST",
                cache: false,
                data: {
                    "_token": "{{ csrf_token() }}",
                ...
                $('#menu-menu-lg').html(data);
            }).fail(function() {
                $('#menu-menu-lg').html('eror');
            });
        });
        $(document).on("click", "#button-setup-akses", function(e) {
            ...
            $.ajax({
                url: "{{ route('master_menu_access_setting_change') }}",
                type: "POST",
                cache: false,
                data: {
                    "_token": "{{ csrf_token() }}",
