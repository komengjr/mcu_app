...
<script>
    ...
    $(document).on("click", "#button-data-location-company", function(e) {
        e.preventDefault();
        var code = $(this).data("code");
        $('#menu-company-full').html(
            '<div class="spinner-border my-3" style="display: block; margin-left: auto; margin-right: auto;" role="status"><span class="visually-hidden">Loading...</span></div>'
        ...
        $.ajax({
            url: "{{ route('master_company_data_location_company') }}",
            type: "POST",
            cache: false,
            data: {
                "_token": "{{ csrf_token() }}",
    ...
    $(document).on("click", "#button-tambah-lokasi-perusahaan", function(e) {
        e.preventDefault();
        var code = $(this).data("code");
        $('#menu-table-location').html(
            '<div class="spinner-border my-3" style="display: block; margin-left: auto; margin-right: auto;" role="status"><span class="visually-hidden">Loading...</span></div>'
        );
        $.ajax({
            url: "{{ route('master_company_data_location_company_add') }}",
            type: "POST",
            cache: false,
            data: {
                "_token": "{{ csrf_token() }}",
            ...
            $('#menu-table-location').html(data);
        }).fail(function() {
            $('#menu-table-location').html('eror');
        });
    });
    $(document).on("click", "#button-tambah-handle-cabang-company", function(e) {
        e.preventDefault();
        var code = $(this).data("code");
        $('#menu-table-location').html(
            '<div class="spinner-border my-3" style="display: block; margin-left: auto; margin-right: auto;" role="status"><span class="visually-hidden">Loading...</span></div>'
        ...
        $.ajax({
            url: "{{ route('master_company_data_location_company_add_handle') }}",
            type: "POST",
            cache: false,
            data: {
                "_token": "{{ csrf_token() }}",
